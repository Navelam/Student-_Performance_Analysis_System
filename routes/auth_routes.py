...
def register():
    ...
    if request.method == 'POST':
        ...
        # Debug print
        print(f"Registration attempt - Role: {role}, Dept ID: {department_id}")
        
        # Validation
        if not all([username, email, full_name, password, confirm_password, role, department_id]):
        ...
        if not department:
            flash(f'Invalid department selected. Please choose a valid department.', 'danger')
            return render_template('auth/register.html', 
                                 roles=get_roles(),
                                 departments=departments,
                                 current_year=current_year)
        ...
        if role == 'student':
            ...
            if not all([registration_number, student_id, batch_year]):
        ...
        try:
            # CREATE USER - WITHOUT phone field (User model doesn't have phone)
            user = User(
                username=username,
                email=email,
                full_name=full_name,
            ...
            # IF STUDENT, CREATE STUDENT RECORD WITH PHONE
            if role == 'student':
                ...
                # CREATE STUDENT RECORD - phone IS valid here
                student = Student(
                    registration_number=registration_number,
                    student_id=student_id,
                    name=full_name,
                    email=email,
                    phone=phone,  # Phone goes in Student model
                    user_id=user.id,
                    course_id=course.id,
                    department_id=department.id,
                    admission_year=int(batch_year),
                    admission_date=datetime(int(batch_year), 6, 15).date(),
                    is_active=True
                )
                db.session.add(student)
                print(f"Created student record for {full_name} with phone {phone}")
