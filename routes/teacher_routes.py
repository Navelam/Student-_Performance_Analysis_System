...
def get_monthly_attendance(student_id, subject_id, month, year):
...
def utility_processor():
    ...
    # Get teacher's assigned subjects for sidebar
...
def dashboard():
    ...
    for perf in performances:
        ...
        # Calculate risk with attendance
    ...
    # Prepare performance data - WITH subject_id included
    performance_data = []
    for perf in performances:
        ...
        if student:
            ...
            # IMPORTANT: Include ALL needed fields in the dictionary
            performance_data.append({
                'id': perf.id,
                'student': student,
                'student_id': student.id,  # For student reference
                'subject_id': perf.subject_id,  # For the edit link - THIS IS CRITICAL
    ...
    # Debug print to verify subject_id is in the data
    if performance_data:
        print(f"DEBUG: First performance has subject_id: {performance_data[0].get('subject_id')}")
...
def all_students():
    ...
    ).order_by(Student.current_semester, Student.name).all()
...
def students_by_year(year):
    ...
    # Get students in teacher's department for the specified semesters
    students = Student.query.filter(
        Student.department_id == current_user.department_id,
        Student.current_semester.in_(semester_range)
    ).order_by(Student.current_semester, Student.name).all()
    
    print(f"DEBUG: Found {len(students)} students for Year {year}")  # For debugging
    ...
    for student in students:
        ...
        if perf:
            ...
            # Student exists but no performance data yet
            student_data.append({
                'id': student.id,
                'student_id': student.student_id,
                'reg_no': student.registration_number,
...
def student_detail(student_id):
    ...
    # Get attendance for each subject and prepare performance data
    performance_data = []
    suggestions = []
...
def student_results(subject_id):
    ...
    for perf in performances:
        attendance, total, attended = get_student_attendance(
            perf.student_id, subject_id, subject.semester_id
        )
...
# =====================================================
# MARKS MANAGEMENT - ENHANCED WITH NAVIGATION
# =====================================================
...
@teacher_required
def marks():
    """Marks overview page with subject selection"""
    # Get teacher's assigned subjects
    assignments = TeacherSubject.query.filter_by(
        teacher_id=current_user.id,
        is_active=True
    ...
    for assignment in assignments:
        ...
        if subject:
            ...
            marked_students = StudentPerformance.query.filter_by(
            ...
            subject_info = {
                ...
                'progress': int((marked_students / total_students * 100)) if total_students > 0 else 0
            }
            
            if subject.semester_id <= 2:
                subjects_by_year['1st Year'].append(subject_info)
            elif subject.semester_id <= 4:
                subjects_by_year['2nd Year'].append(subject_info)
            elif subject.semester_id <= 6:
                subjects_by_year['3rd Year'].append(subject_info)
            else:
                subjects_by_year['4th Year'].append(subject_info)
    
    return render_template('teacher/marks_overview.html',
                         subjects_by_year=subjects_by_year)

@teacher_bp.route('/marks/<int:subject_id>')
@login_required
@teacher_required
def marks_list(subject_id):
    """View all students with their marks for a subject"""
    # Verify teacher is assigned to this subject
    assignment = TeacherSubject.query.filter_by(
        teacher_id=current_user.id,
        subject_id=subject_id,
    ...
    # Prepare data for template - create a list of dictionaries with all needed fields
    performance_data = []
    for perf in performances:
        # Get attendance for this student
        attendance, total, attended = get_student_attendance(perf.student_id, subject_id, subject.semester_id)
        ...
        performance_data.append({
            ...
            'assessment': perf.assessment,
            'total_marks': perf.total_marks,
            'final_internal': perf.final_internal,
    ...
    return render_template('teacher/student_results.html',
                         ...
                         marks_entered=stats['entered'],
                         pending=stats['pending'])

@teacher_bp.route('/marks/enter/<int:subject_id>', methods=['GET'])
@login_required
@teacher_required
def enter_marks(subject_id):
    """Show manual entry form for marks with navigation"""
    # Verify teacher is assigned to this subject
    assignment = TeacherSubject.query.filter_by(
        teacher_id=current_user.id,
        subject_id=subject_id,
    ...
    # Get all students for this subject (for dropdown)
    students = Student.query.filter_by(
        department_id=subject.department_id,
        current_semester=subject.semester_id
    ).order_by(Student.name).all()
    
    # Get selected student from query param (optional)
    ...
    if selected_student:
        next_student = get_next_student(selected_student.id, subject_id)
        prev_student = get_previous_student(selected_student.id, subject_id)
        current_index = get_student_index(selected_student.id, subject_id)
    ...
    return render_template('teacher/enter_marks.html',
                     ...
                     total_students=total_students,
                     stats=stats,  # This is important
                     existing_marks=existing_marks)  # This is what the template expects
...
def save_marks(subject_id):
    ...
    action = request.form.get('action', 'save')  # 'save' or 'save_next'
...
@teacher_bp.route('/marks/bulk/<int:subject_id>', methods=['GET', 'POST'])
@login_required
@teacher_required
def bulk_marks_entry(subject_id):
    ...
    if request.method == 'POST':
        ...
        success_count = 0
        for student in students:
            internal1 = int(request.form.get(f'internal1_{student.id}', 0))
            internal2 = int(request.form.get(f'internal2_{student.id}', 0))
            seminar = int(request.form.get(f'seminar_{student.id}', 0))
            assessment = int(request.form.get(f'assessment_{student.id}', 0))
            ...
            if existing:
                existing.internal1 = internal1
                existing.internal2 = internal2
                existing.seminar = seminar
                existing.assessment = assessment
                existing.total_marks = total_marks
                existing.final_internal = final_internal
            else:
                performance = StudentPerformance(
                    student_id=student.id,
                    subject_id=subject_id,
    ...
    if academic_year:
        ...
        for perf in performances:
            existing_marks[perf.student_id] = perf
    
    return render_template('teacher/bulk_marks.html',
                         subject=subject,
                         students=students,
...
def save_attendance(subject_id):
    """Save monthly attendance for students"""
    # Verify teacher is assigned to this subject
    assignment = TeacherSubject.query.filter_by(
        teacher_id=current_user.id,
        subject_id=subject_id,
    ...
    for i, student_id in enumerate(student_ids):
        try:
            ...
            if existing:
                ...
                existing.penalty_status = penalty_status
...
def attendance_report(subject_id):
    """View attendance report for a subject"""
    # Verify teacher is assigned to this subject
    assignment = TeacherSubject.query.filter_by(
        teacher_id=current_user.id,
        subject_id=subject_id,
    ...
    for student in students:
        ...
        if record:
            ...
            attendance_list.append(record.attendance_percentage)
            penalty_total += record.penalty_amount
        else:
            display_data.append({
                'student': student,
                'attendance': None
    ...
    warning_count = sum(1 for a in attendance_list if 70 <= a < 75) if attendance_list else 0
    good_count = sum(1 for a in attendance_list if a >= 75) if attendance_list else 0
    
    # ML Analysis
    ml_analysis = {
        'average': round(avg_attendance, 1),
        'safe_count': good_count,
        'moderate_count': warning_count,
        'high_risk_count': 0,
        'critical_count': critical_count,
        'risk_percent': round(((critical_count + warning_count) / len(attendance_list) * 100) if attendance_list else 0, 1)
    }
    ...
    return render_template('teacher/attendance_report.html',
                         ...
                         total_students=len(students),
                         recorded_count=len(attendance_records),
...
def risk_alerts():
    ...
    for perf in performances:
        ...
        if risk == 'Critical' or attendance < 70:
            ...
            risk_students.append(risk_item)
    
    # Sort by marks (lowest first)
...
def add_student():
    ...
    if request.method == 'POST':
        ...
        # Get course (first course in department)
...
def upload_question_paper(subject_id):
    ...
    if request.method == 'POST':
        ...
        # If academic_year not provided, calculate current academic year
        if not academic_year:
            from datetime import date
            today = date.today()
            if today.month >= 6:
...
def delete_question_paper(paper_id):
    ...
    # Check if teacher owns this paper or is assigned to the subject
    assignment = TeacherSubject.query.filter_by(
        teacher_id=current_user.id,
